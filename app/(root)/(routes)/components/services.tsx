      ...
      <ul className="flex flex-wrap gap-2 md:gap-3 items-center">
        {servicesCategories.map(({ name, slug, icon: Icon }) => (
          <>
            <li>
              <Link
                className="uppercase font-ao text-dark dark:text-light text-[14px] flex items-center gap-x-2 hover:bg-purple/25 px-4 py-2 rounded-xl w-max transition duration-200"
                href={`services/${slug}`}
              >
                {Icon && (
                  <span className="text-purple text-[18px]">
                    <Icon />
                  </span>
                )}
                {name}
